...
async def fetch_twelvedata_async(client: httpx.AsyncClient, symbol: str = "XAU/USD", interval: str = "5min", outputsize: int = 2500) -> pd.DataFrame:
    """Fetches real-time, zero-delay Spot Gold asynchronously from TwelveData."""
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"{Color.RED}TwelveData API Status Error: {res.status_code}{Color.RESET}")
            return pd.DataFrame()
        ...
        print(f"{Color.RED}TwelveData Async Exception: {e}{Color.RESET}")
        return pd.DataFrame()

async def build_macro_matrix_async(client: httpx.AsyncClient, daysback: int = 7) -> pd.DataFrame:
    print(f"{Color.CYAN}📥 Building Macro-Synchronized Matrix Asynchronously...{Color.RESET}")
    candles_needed = daysback * 288
    
    # Offload blocking yfinance call to an executor thread to preserve async loop runtime
    ...
    if not dxy_df.empty:
        ...
        dxy_df.index = dxy_df.index.tz_localize('UTC') if dxy_df.index.tz is None else dxy_df.index.tz_convert('UTC')
...
async def fetch_live_candle_async(client: httpx.AsyncClient) -> Optional[pd.Series]:
    """Pings TwelveData asynchronously for zero-delay Spot Gold and merges DXY."""
    try:
        loop = asyncio.get_running_loop()
        dxy_future = loop.run_in_executor(None, lambda: yf.Ticker("DX-Y.NYB").history(period="1d", interval="5m"))
        
        gold, dxy = await asyncio.gather(
            fetch_twelvedata_async(client, symbol="XAU/USD", outputsize=30),
            dxy_future
        )
        ...
        if not gold.empty:
            ...
            if not dxy.empty:
                dxy.index = dxy.index.tz_localize('UTC') if dxy.index.tz is None else dxy.index.tz_convert('UTC')
            ...
            if live_features.empty: return None
            return live_features.iloc[-2]
            
        return None
    except Exception as e:
        print(f"Live Async API Error: {e}")
        return None