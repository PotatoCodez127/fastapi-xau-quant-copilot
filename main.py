...
async def run_live_forward_testing_loop():
    logger.info(
        f"{Color.CYAN}🟢 INITIALIZING ASYNC XAUUSD LIVE TELEMETRY CORE...{Color.RESET}"
    )
    ...
    async with httpx.AsyncClient() as client:
        ...
        knowledge_graph = build_knowledge_graph(
            generate_mock_trade_history(num_trades=2000)
        )
        ...
        while True:
            try:
                ...
                if (
                    ...
                    and latest_candle.name > last_processed_time
                ):
                    ...
                    if (
                        ...
                        if decision_json and decision_json.get("Decision") == "EXECUTE":
                            ...
                            tracker.open_trade(
                                timestamp=int(latest_candle.name.timestamp()),
                                direction=direction,
