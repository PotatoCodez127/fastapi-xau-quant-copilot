...
async def run_live_forward_testing_loop():
    logger.info(f"{Color.CYAN}🟢 INITIALIZING ASYNC XAUUSD LIVE TELEMETRY CORE...{Color.RESET}")
    ...
    async with httpx.AsyncClient() as client:
        ...
        knowledge_graph = build_knowledge_graph(generate_mock_trade_history(num_trades=2000))
        ...
        logger.info(f"{Color.GREEN}✅ Live Telemetry Fully Armed. Loop Initiated.{Color.RESET}")
        
        while True:
            try:
                latest_candle = await fetch_live_candle_async(client)
                if latest_candle is not None and latest_candle.name > last_processed_time:
                    last_processed_time = latest_candle.name
                    ...
                    if tracker.active_trade is None and QuantitativeGuard.verify_execution_threshold(latest_candle):
                        ...
                        market_state = f"Time: {latest_candle.name.strftime('%H:%M UTC')} ({latest_candle.name.strftime('%A')})\nSession: {current_session}\nStrategy: {current_strat}\nTape: {current_tape}"
                        decision_json = await evaluate_trade_setup_async(client, market_state, rag_context, graph_context)
                        ...
                        if decision_json and decision_json.get("Decision") == "EXECUTE":
                            ...
                            sl_distance, tp_distance = (6.0, 12.0) if sl_type == "Wide_SL" else ((2.0, 4.0) if sl_type == "Tight_SL" else (4.0, 8.0))
                            ...
                            tracker.open_trade(
                                timestamp=int(latest_candle.name.timestamp()), direction=direction,
                            ...
                            await state_manager.broadcast('trade_signal', {
                ...
                # Explicit exception naming enforces compliance with Ruff check rules (E722)
