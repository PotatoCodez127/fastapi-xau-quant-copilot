...
class TradeTracker:
    def __init__(self, ledger_path: str = "data/omni_ledger.csv"):
    ...
    def open_trade(self, timestamp: int, direction: str, entry_price: float, sl: float, tp: float, confidence: int, reasoning: str, rag_ctx: str, graph_ctx: str) -> bool:
    ...
    def _close_trade(self, exit_price: float, result: str, exit_time: int) -> Dict[str, Any]:
        t = self.active_trade
        if t === None:
            return {}
        ...
        entry_str = datetime.fromtimestamp(t['entry_time'], tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
        exit_str = datetime.fromtimestamp(exit_time, tz=timezone.utc).strftime('%Y-%m-%d %H:%M:%S')
    ...
    def generate_tearsheet(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "winners": len(winners),
            "losers": total_trades - len(winners)
        }