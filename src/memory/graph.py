...
def generate_mock_trade_history(num_trades=1000):
    ...
    """
    print(
        f"{Color.CYAN}📊 Generating Synthetic Trade History ({num_trades} trades)...{Color.RESET}"
    )
...
def build_knowledge_graph(df):
    ...
    print(
        f"{Color.GREEN}✅ Graph Built: {G.number_of_nodes()} Nodes, {G.number_of_edges()} Edges.{Color.RESET}\n"
    )
    return G
...
def evaluate_topology(G, current_session, current_strat):
    ...
    """
    print(
        f"{Color.MAGENTA}🧭 Querying Graph Topology for: [{current_session} -> {current_strat}]{Color.RESET}"
    )
    print("-" * 50)
    ...
    for node, attr in G.nodes(data=True):
        if (
            ...
            if win_rate >= 60:
                insights.append(
                    f"🟢 SAFEPATH DETECTED: {node} ({win_rate:.1f}% Win Rate | {wins}W - {losses}L)"
                )
            elif win_rate <= 40:
                insights.append(
                    f"🔴 DANGER NODE: {node} ({win_rate:.1f}% Win Rate | {wins}W - {losses}L)"
                )
            else:
                insights.append(
                    f"⚪ NEUTRAL PATH: {node} ({win_rate:.1f}% Win Rate | {wins}W - {losses}L)"
                )
...
if __name__ == "__main__":
    ...
    evaluate_topology(
        knowledge_graph, current_session="NY_London_Overlap", current_strat="Breakout"
    )
    ...
    evaluate_topology(
        knowledge_graph, current_session="London_Open", current_strat="Mean_Reversion"
    )
