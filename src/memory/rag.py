...
def generate_future_outcome(df: pd.DataFrame, current_idx: Any, forward_look: int = 12) -> str:
    ...
    try:
        ...
        if future_idx >= len(df): return "Unknown (End of Data)"
...
def setup_chroma_db() -> chromadb.Collection:
    ...
    return client.get_or_create_collection(name="xau_memory_bank", metadata={"hnsw:space": "cosine"})
