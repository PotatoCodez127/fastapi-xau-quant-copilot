...
class Color:
    CYAN, RED, YELLOW, GREEN, MAGENTA, RESET = (
        "\033[96m",
        "\033[91m",
        "\033[93m",
        "\033[92m",
...
def run_deep_diagnostics(ledger_path="data/omni_ledger.csv"):
    ...
    try:
        df[["Session", "Strategy", "SL_Type"]] = df["Node"].str.split(
            " \| ", n=2, expand=True
        )
    except:
        df["Session"] = "Unknown"
    ...
    def print_group_stats(group_name, column):
        ...
        for index, row in stats.iterrows():
            ...
            print(
                f"{str(index):.<22} Trades: {row['Trades']:<4} | Win Rate: {row['Win_Rate']:>5.1f}% | Net PnL: {pnl_color}{row['Net_Pips']:>7.1f}{Color.RESET}"
            )
        print()
    ...
    max_loss_streak = (
        df["Loss_Int"]
        .groupby((df["Loss_Int"] != df["Loss_Int"].shift()).cumsum())
        .cumsum()
    ).max()
    ...
    node_stats = (
        df.groupby("Node")
        .agg(Net_Pips=("Pips", "sum"), Trades=("Result", "count"))
        .sort_values(by="Net_Pips")
    )

    print(f"{Color.GREEN}🏆 TOP 3 ALPHA NODES (Most Profitable){Color.RESET}")
    for node, row in (
        node_stats.sort_values(by="Net_Pips", ascending=False).head(3).iterrows()
    ):
        if row["Net_Pips"] > 0:
